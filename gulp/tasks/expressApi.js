...
const npmi= util.promisify(require('npmi'));
...
router.post('/feature', async function (req, res) {
    ...
    let userManifest = await storage.getItem(userId);
    ...
    try {
        await npmi({path: 'primo-explore/custom/' + userId, name: npmId, version: npmVersion, forceInstall: true});
    }
    catch (err){
    ...
    try{
        ...
        await utils.wrapFilesWithAutoGeneratedHeader(['primo-explore/custom/' + userId + '/node_modules/' + npmId +'/**/*.js']);
        ...
        try{
            await Promise.all([appCssPromise, buildCustomJsPromise]);
...
router.get('/remove_feature', async function (req, res) {
    let cookies = utils.parseCookies(req);
    let urlForProxy = cookies['urlForProxy'];
    let userId= utils.getUserId(req);
    let userManifest = await storage.getItem(userId);
    ...
    if (index === -1){
        ...
        try {
            ...
            await Promise.all([rimrafPromise, buildCustomHookJsFilePromise]);
            let appCssPromise = appCss(userId, urlForProxy);
            let buildCustomJsPromise = buildCustomJs.customJs(userId).then(()=>{
                userManifest[hookName] = hookFeatureList;
                storage.setItem(userId, userManifest);
            ...
            console.error('failed to build custom hook js file');
            utils.sendErrorResponse(res, err);
        }
    }
});
...
router.get('/colors', async function(req, res){
    try{
        ...
        let data = await fsp.readFile(baseDir+'/colors.json.txt');
        res.send(data);
    }
    catch (err){
        utils.sendErrorResponse(res, err);
...
router.post('/colors', async function(req, res) {
    try {
        let cookies = utils.parseCookies(req);
        let urlForProxy = cookies['urlForProxy'];
        ...
        await fsp.writeFile(baseDir + '/colors.json.txt', JSON.stringify(colors), {encoding: 'utf-8'});
        console.log('finished writing colors.json.txt');
        await appCss(userId, urlForProxy);
...
router.get('/icons', async function(req, res){
    ...
    if (fileExists){
        ...
        try{
            await fsp.writeFile(customUiPath,
...
router.post('/icons', async function (req, res) {
    let cookies = utils.parseCookies(req);
    let urlForProxy = cookies['urlForProxy'];
