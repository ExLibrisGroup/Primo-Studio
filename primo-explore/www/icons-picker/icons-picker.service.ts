...
export class IconsPickerService {
  private _icons: {[key: string]: Icon};
  private _defaultIcons: {[key: string]: Icon};
  private _thirdPartyIcons: {[thirdParty: string]: Array<IconInformation>} = {};
  ...
  constructor(private $http: HttpClient,
    ...
    this.$http.get(IconType.PRIMO_UI, {
      responseType: 'text'
    }).subscribe(response => {
        this._defaultIcons = this.parseSvg(response);
      }, () => {
        console.log('failed to get default icons from server, using default icons');
      });

    for (let iconType of [IconType.FONT_AWESOME, IconType.BOOTSTRAP]) {
      this.$http.get(iconType, {responseType: "text"}).subscribe(fontFile => {
        this.$http.post('/optimize-svg', {data: fontFile}).subscribe((characterSvgs: Array<IconInformation>) => {
          this._thirdPartyIcons[iconType] = characterSvgs;
        });
      });
    }
  }

  public loadIconsFromServer(): void {
    this.$http.get('/icons', {
      responseType: 'text'
    }).subscribe((response) => {
      this._icons = this.parseSvg(response);
    }, () => {
  ...
  parseSvg(svgFile: string) {
    let svg = new DOMParser().parseFromString(svgFile, 'application/xml');
    let svgNodeList = svg.querySelectorAll<SVGSVGElement>('svg[id]');
    let svgJson :{[key: string]: Icon} = {};

    for (let i = 0; i < svgNodeList.length; i++) {
      let element = svgNodeList[i];
      svgJson[element.id] = {
        'name': element.id,
        'id': element.id,
  ...
  extractPath(id: string, icon: Icon): void {
    if (icon.path) {
        ...
        if (this._thirdPartyIcons[icon.type]){
          ...
          setTimeout(() => {
            if (this._thirdPartyIcons[icon.type]){
              ...
              console.log('Network is slow, please try again.');
            }
          }, 3000);
        }
      }).subscribe(() => {
        this._icons[id].path = this._thirdPartyIcons[icon.type].filter(iconInformation => iconInformation.code === hexCode)[0].svg;
      });
    }
  }
    ...
    if (ipIconPack === 'prm' || ipIconPack === 'all') {
      let prmIcons = _keys(this._defaultIcons).map(key => {
        ...
        switch (key) {
          ...
          case 'sign-in':
          case 'sign-out':
            filters.push('authentication', 'user');
            break;
          case 'magnifying-glass':
            filters.push('search');
            break;
          case 'filterOut':
            filters.push('not filter', 'remove');
            break;
          case 'hamburger':
          case 'toggle-list':
          case 'toggle-items':
          ...
          case 'beer':
          case 'sun':
            filters.push('my account', 'beach');
            break;
          case 'bell':
            filters.push('alert');
            break;
          case 'trash':
            filters.push('garbage');
            break;
          case 'restore':
          ...
          case 'refworks':
          case 'ris':
          case 'prm_ic_description_24px':
          case 'prm_ic_content_copy_24px':
            filters.push('page');
            break;
        }
        icon.filter = icon.filter.concat(filters);
        return icon;
      });
